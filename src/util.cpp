...
{
    namespace fs = boost::filesystem;
    // Windows: C:\Documents and Settings\username\Application Data\Dystater
    // Mac: ~/Library/Application Support/Dystater
    // Unix: ~/.dystater
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "Dystater";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-conf", "dystater.conf"));
...
boost::filesystem::path GetPidFile()
{
    ...
    fs::path pathPidFile(GetArg("-pid", "dystaterd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Dystater will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("Dystater"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
